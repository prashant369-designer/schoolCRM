...
function Navbar({ panelName = "School ERP", panelType = "Student Panel" }) {
  ...
  const classInfo = details?.class_name
    ? `${details.class_name}-${details.section || ""}`
    : role || "Admin Panel";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
      <nav className="flex h-16 items-center justify-between px-4 md:px-6">
        ...
        {/* Search div */}
        <div className="hidden md:flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-2 text-sm font-medium outline-none transition focus-within:border-indigo-300 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-50">
          <Search size={18} className="text-slate-500" />

          <input
            className="bg-transparent outline-none w-64"
            type="text"
            placeholder="Search teacher, student, etc"
          />
        </div>
                ...
                <div className="bg-linear-to-r from-indigo-600 to-sky-500 p-5 text-white">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/80">{classInfo}</p>
